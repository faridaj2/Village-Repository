    ...
    <body class="font-sans antialiased bg-white text-gray-900">
        ...
        <nav x-data="{ scrolled: false }" @scroll.window="scrolled = window.scrollY > 20"
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16 sm:h-20">
                <a href="/" class="flex items-center gap-2.5">
                    <img src="{{ asset('logo-tagline.png') }}" alt="SIDESA" class="h-9 sm:h-10 w-auto">
                </a>
                <div class="hidden md:flex items-center gap-8">
                    <a href="#pengumuman" class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">Pengumuman</a>
                    <a href="#fitur" class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">Fitur</a>
                    <a href="#tentang" class="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">Tentang</a>
                </div>
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-28 sm:py-32 w-full">
                <div class="max-w-2xl lg:max-w-3xl text-center mx-auto">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-3 fade-up delay-300">
                        ...
                        @guest
                            <a href="{{ route('login') }}" class="inline-flex items-center gap-2 px-7 py-3.5 text-sm font-semibold text-white bg-white/10 backdrop-blur-md border border-white/20 rounded-xl hover:bg-white/20 transition-all">
        ...
        <section class="relative -mt-16 z-20 px-4 sm:px-6">
            <div class="max-w-7xl mx-auto">
                <div class="bg-white rounded-3xl shadow-2xl shadow-gray-900/10 border border-gray-100 p-6 sm:p-8 grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                    @php
                        $stats = [
                            ['label' => 'Penduduk Aktif', 'value' => number_format($totalPenduduk, 0, ',', '.'), 'color' => 'indigo', 'icon' => 'users'],
                            ['label' => 'Kartu Keluarga', 'value' => number_format($totalKk, 0, ',', '.'), 'color' => 'emerald', 'icon' => 'home'],
                            ['label' => 'Surat Terbit', 'value' => number_format($totalSurat, 0, ',', '.'), 'color' => 'amber', 'icon' => 'mail'],
                            ['label' => 'Total Rumah', 'value' => number_format($totalRumah, 0, ',', '.'), 'color' => 'purple', 'icon' => 'building'],
                        ];
                    @endphp
                    @foreach ($stats as $s)
        ...
        <section id="fitur" class="py-24 px-4 sm:px-6">
            <div class="max-w-7xl mx-auto">
                ...
                @php
                    $fiturs = [
                        ['title' => 'Data Penduduk', 'desc' => 'Kelola data penduduk lengkap dengan kartu keluarga, rumah, dan wilayah.', 'color' => 'indigo', 'icon' => 'users'],
                        ...
                        ['title' => 'Media & Dokumen', 'desc' => 'Simpan dan kelola file serta dokumen penting desa dengan aman.', 'color' => 'sky', 'icon' => 'folder'],
                    ];
                @endphp
            ...
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-12 gap-4">
                    <div>
                        ...
                        <p class="text-gray-500 mt-2">Informasi dan berita terbaru dari Desa Waeleman</p>
                    </div>
                    <a href="{{ route('pengumuman.publik') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors group">
                        Lihat Semua
                        <svg class="w-4 h-4 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                    </a>
                </div>
                        ...
                        @foreach ($pengumumans as $i => $item)
                            <a href="{{ route('pengumuman.publik') }}#pengumuman-{{ $item->id }}"
                               class="group relative bg-white rounded-3xl border border-gray-100 p-7 hover:shadow-2xl hover:border-indigo-100 hover:-translate-y-1 transition-all duration-300 block">
                                ...
                                <h3 class="text-lg font-bold mb-2 line-clamp-2 group-hover:text-indigo-600 transition-colors">{{ $item->judul }}</h3>
                                <p class="text-sm text-gray-500 line-clamp-3 leading-relaxed">{{ strip_tags($item->isi) }}</p>
                ...
                @else
                    <div class="text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200">
                        ...
                        <p class="text-sm font-semibold text-gray-900 mb-1">Belum ada pengumuman</p>
        ...
        {{-- CTA --}}
        <section id="tentang" class="py-24 px-4 sm:px-6">
            <div class="max-w-7xl mx-auto">
                <div class="relative bg-gradient-to-br from-gray-900 via-indigo-950 to-purple-950 rounded-[2.5rem] overflow-hidden p-10 sm:p-16 lg:p-20 text-center">
                    ...
                    <div class="relative">
                        <h2 class="text-3xl sm:text-5xl font-black text-white tracking-tight mb-4">Siap Melayani Warga<br>Dengan Lebih Baik</h2>
